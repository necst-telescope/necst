...
def test_preflight_logs_single_line_edge_slowdown_and_allows_execution(monkeypatch):
    ...
    monkeypatch.setattr(MODULE, "plan_scan_block_kinematics", lambda lines: {
        ...
        "lines": [{
            "line_index": 0, "label": "L0", "required_acceleration": q(2.45, "deg/s^2"),
            "peak_acceleration": q(1.60, "deg/s^2"), "peak_jerk": q(4.2, "deg/s^3"),
            "duration_scale": q(1.5, ""), "within_limits": True,
        }],
        "turns": [],
    })
    obs._run_on_scan_block([Waypoint("L0")], scan_frags=[1], cos_scan=False, margin_deg=0.1, include_final_standby=False, final_standby_duration_sec=1.0)
    assert any("line-edge slowed for kinematic limits" in msg for msg in obs.logger.infos)
...
def test_preflight_logs_turn_slowdown_and_allows_execution(monkeypatch):
    obs = _make_obs()
    line0 = SimpleNamespace(start=(q(0.0), q(0.0)), stop=(q(1.0), q(0.0)), speed=q(0.5, "deg/s"), margin=q(0.1), label="L0", line_index=0)
    ...
    monkeypatch.setattr(MODULE, "plan_scan_block_kinematics", lambda lines: {
        "limits": SimpleNamespace(max_acceleration=q(1.6, "deg/s^2")),
        "lines": [
            {"line_index": 0, "label": "L0", "required_acceleration": q(2.34, "deg/s^2"), "peak_acceleration": q(1.5, "deg/s^2"), "duration_scale": q(1.25, ""), "within_limits": True},
            {"line_index": 1, "label": "L1", "required_acceleration": q(2.34, "deg/s^2"), "peak_acceleration": q(1.5, "deg/s^2"), "duration_scale": q(1.25, ""), "within_limits": True},
        ],
        "turns": [
            {
                "from_line_index": 0,
    ...
    obs._run_on_scan_block([Waypoint("L0"), Waypoint("L1")], scan_frags=[1, -1], cos_scan=False, margin_deg=0.1, include_final_standby=False, final_standby_duration_sec=1.0)
    assert any("line-edge slowed for kinematic limits" in msg for msg in obs.logger.infos)
