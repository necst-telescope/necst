# NECST/XFFTS spectral recording 累積簡易マニュアル update14

Version: update14 / 日本語版 / 2026-05-03  
対象: original zip群に、update14本体および2026-05-03までの実機preflight修正を適用したtree。

## 1. 最短要点

- `lo_profile.toml` のSG周波数は `sg_set_frequency_ghz`, `sg_set_frequency_mhz`, `sg_set_frequency_hz` のいずれか1つで書ける。
- fixed LOは `fixed_lo_frequency_ghz` を使う。`frequency_ghz` ではない。
- 200 GHz帯1st localが制御対象でなく225 GHz固定なら、以下だけでよい。
...
- `sg_devices.<id>` と `lo_roles.<role_id>` が同名でも、現実装では `sg_id` を明示する。
- 実行権限問題を避けるため、基本は `necst lo_profile ...` 形式を使う。
- setup modeでは `ch=None`, `tp_mode=False`, `tp_range=[]` はlegacy指定として扱わない。
- single-window `contiguous_envelope` では top-level `line_name` / `window_id` を記録する。
- multi-windowのDB pathは `data/spectral/xffts/board2__13CO...` のように短縮された。
- converter/sunscanはsnapshotの `db_table_path` / `db_table_name` を優先して読むよう修正済み。
- NECSTDB writerは `boolean` field typeに対応済み。
...
necst lo_profile apply lo_profile.toml --verify --timeout-sec 10
```

standaloneの `necst-lo-profile` も残しているが、zip展開時に実行ビットが落ちる場合があるため、`necst lo_profile ...` を推奨する。

## 3. setup観測

`.obs/.toml` 側にspectral recording setupを指定している場合、旧式 `ch`, `tp_mode`, `tp_range` を実値で併用しない。
...
## 4. 旧converter config 4 board例

`docs/examples/legacy_converter_20260503/` を参照する。200 GHz帯1st localは225 GHz fixed LOとして書く。

## 5. converter/sunscan

converter/sunscanはDB内snapshotを自動検出する。新path短縮後も、snapshotの `db_table_path` を正として読む。

```bash
necst_v4_sdfits_converter <RawDataDir> --analysis-config converter_analysis_...toml --inspect-spectral-time
```

sunscanはconverter analysis TOMLをbaseにして差分だけ書く。
...
msg/srvを変えていないpatchでは `necst_msgs` rebuildは不要。ただし実行時に `build/necst/...` を読んでいる場合は、Python変更反映のため `necst` だけ再buildする。
...
`saved_nchan` に比例したファイルサイズになっていれば、window切り出し保存は概ね正常である。

---

## 8. update13までの簡易manual本文

以下にupdate13までの簡易manual情報を保持する。
...
この簡易版は、original zip群からupdate14までの累積変更を使うための最短手順である。

## 1. 最短要点

- 観測時の真実はDB内`spectral_recording_snapshot.toml`に残す。
- converter/sunscanはDB内snapshotを自動検出する。
- XFFTS timestampは積分中心時刻として扱う。
- DB spectral row `time`はheader受信時刻、`recorded_time`はwriter時刻。
- OMU/XFFTSではliteral `GPS` suffixでもdefaultはUTC相当として読む。
- converter analysis TOMLをbaseにし、sunscanは`[analysis_base]`で差分だけ書く。
- `[spectral_time]`/`[encoder_time]`はトップレベルに書き、analysis section内へ混ぜない。

## 2. converter/sunscanの形
...
spectrometer_time_offset_sec = -0.145  # host-time fallback時だけ
...
[converter_analysis]
# converter固有設定と共通DB/source/table/chopper/weather設定
```
...
[sunscan_analysis]
# sunscan固有のfit/trimming/report設定だけ
```
...
PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 python -m pytest -q \
  sd-radio-spectral-fits-main/tests/test_sunscan_analysis_base_config.py \
  sd-radio-spectral-fits-main/tests/test_converter_spectral_time_controls_p16g.py \
  sd-radio-spectral-fits-main/tests/test_necst_config_separation_snapshot_bundle.py
```

期待結果は`14 passed`。

## 4. update13までの簡易manualを削らず収録

以下はupdate13 quick manualである。update14で更新された時刻設定とsunscan analysis baseは上の1-3章を優先する。
...
source treeの `bin` をPATHに入れて使う運用では、zip更新時に `necst-lo-profile` などの実行ビットが落ちることがある。その場合でも、元からある `bin/necst` wrapper経由で次のように実行できる。`bin/necst` は `bin/*.py` を `python3` で起動するため、個別の `.py` に実行権限は不要である。
...
`colcon build` 後に `install/setup.bash` をsourceして使う場合は、console scriptとして `necst-lo-profile` / `necst-spectral-resolve` / `necst-spectral-validate` も使える。update14のzipではこれらのstandalone wrapperにも実行ビットを付けているが、展開ツールが権限を保持しない場合の安全な代替として `necst lo_profile` 形式を用意している。
...
`sg_set_frequency_hz` / `sg_set_frequency_mhz` / `sg_set_frequency_ghz` はSGへ直接設定する周波数であり、逓倍後のphysical LOではない。TOMLではいずれか1つだけを書く。内部ではHzへ正規化し、`Commander.signal_generator(..., GHz=...)` へ渡す。
...
- 実際の `db_table_path` は `data/spectral/xffts/board2__13CO_J2_1` のように、spectrometer namespace配下では `boardN__window_id` を使う。
- single-windowの `contiguous_envelope` では、top-levelにも `line_name`, `window_id`, `rest_frequency_hz` を保持する。

## 2026-05-04 追補

- SD FITS出力時の周波数軸は `CRPIX1=1`, `CRVAL1=sky(saved_ch_start)`, `CDELT1=if_frequency_sign*if_freq_step_hz` とする。
- `saved_ch_start:saved_ch_stop` は半開区間であり、`saved_nchan = saved_ch_stop - saved_ch_start`。
- 12CO/13CO/C18Oなど複数lineを同一SINGLE DISH tableへ出す場合に備え、rowごとの `RESTFREQ` / `VELDEF` を出力する。
- 実機snapshotの6 streamで、保存window境界の1 channelずれは見つかっていない。
- beam modelのoptional fieldに空文字があっても、converterでは未指定として扱う。

## 2026-05-04 追補: VELDEF

- converter/writer内部の意味は `veldef = "RADIO"`, `specsys = "TOPOCENT"`。
- SDFITS table の `VELDEF` 列では、writerがSDFITS/AIPS系表現へ正規化し `RADI-OBS` として保存する。
- したがって、testでは `RADIO` ではなく `RADI-OBS` を期待する。
- 確認すべき本質は、topocentric FREQ axisでも row-level `RESTFREQ` / `VELDEF` が出ることである。

## 2026-05-04 追補: chopper設定

- converterは `chopper_wheel/chopper_stat/chopper_win_sec` を使わない。
- chopper設定はsunscan専用として `[sunscan_analysis]` に置く。
- `chopper_win_sec` は現在の1-load sunscan実装ではlegacy互換optionであり、通常はコメントアウトする。

## 2026-05-04 追補: examples整理

- 新しいNANTEN2 multibeam例は `docs/examples/NANTEN2_multibeam_260331TO/` を使う。
- 新しいOMU 1.85 m例は `docs/examples/OMU1P85M_single115_xx/` を使う。
- `old_config_260331TO_update11/` と `legacy_converter_20260503/` は履歴用。
- converterのstream選択は `spectral_name` ではなく `--recorded-stream-id` または `[analysis_selection]` を使う。
- converterはchopper設定を使わない。chopper設定はsunscan側に置く。

## 2026-05-04 追補: spectral_time_source

- examplesの既定は `spectral_time_source = "auto"`。
- 選択肢は `"auto"`, `"xffts-timestamp"`, `"host-time"`。
- `auto` はXFFTS timestampを優先し、使えないときだけhost-timeにfallbackする。
- `spectrometer_time_offset_sec` はhost-timeが実際に選ばれた場合だけ効く。`auto`でXFFTS timestampが選ばれた場合は効かない。
- `xffts_timestamp_scale` は `"auto"`, `"utc"`, `"gps"`, `"tai"`。
- `xffts_gps_suffix_means` は `"utc"` または `"gps"`。OMU/XFFTSでは `"utc"` 推奨。

## 2026-05-12 追補: OMU 1.85 m 4-board example

- 現行4-board setupを `docs/examples/OMU1P85M_4board_12co_13co_c18o_current/` に追加。
- `fdnum=1`: 200 GHz準光学path。
- `fdnum=2`: 100 GHz準光学path。
- `ifnum`: board1--4で `0,1,2,3` として重複なし。
- `plnum=0`: 全stream共通。
- 200 GHz first localは `source="fixed"`, `fixed_lo_frequency_ghz=225.0`。

## 2026-05-12 追補: multi_windowのfdnum/ifnum/plnum

- `recording_groups.*.windows[]` に `fdnum`, `ifnum`, `plnum` を書けるようにした。
- これはsaved productだけを上書きし、source streamは変更しない。
- OMU 1.85 m現行例では6 productに `ifnum=0..5` を割り当てた。

## 2026-05-12 追補: fdnumごとのifnumと静止周波数

- `IFNUM` はglobal uniqueではなく、`FDNUM`ごとに `0,1,2` と数える。
- product識別は `(FDNUM, IFNUM, PLNUM)` の組で行う。
- OMU 1.85 m現行例では全recording groupを `saved_window_policy = "multi_window"` にし、single-line boardでも `window_id` をファイル名/DB pathへ反映する。
- 採用静止周波数を以下に更新した。
  - 12CO(1-0): 115.2712018 GHz
  - 12CO(2-1): 230.5380000 GHz
  - 13CO(1-0): 110.2013543 GHz
  - 13CO(2-1): 220.3986842 GHz
  - C18O(1-0): 109.7821734 GHz
  - C18O(2-1): 219.5603541 GHz

## 2026-05-12 追補: examples統合

- `OMU1P85M_single115_xx/` は重複のため削除対象。
- 現行OMU 1.85 m例は `OMU1P85M_4board_12co_13co_c18o_current/` に統合。
- single115側のconverter/sunscan analysis TOMLは、current 4-board directoryへrenameして移動。
- 既存checkoutへ適用する場合は `rm -rf docs/examples/OMU1P85M_single115_xx` を実行。
- NANTEN2 recording exampleにもwindow-level `ifnum` override例を追加。
- NANTEN2では `fdnum/plnum` はsource streamから継承し、windowでは固定しない。

## 2026-05-12 追補: Sun/sunscan TP

- 太陽観測は連続波TPなので、速度windowではなくchannel指定を使う。
- 追加example:
  - `recording_window_setup_sun_tp_OMU1P85M_4board_current.toml`
  - `recording_window_sun_tp_NANTEN2_multi_260331TO.toml`
- 既定channel範囲は `8192:24576`。
- `mode = "tp"`, `saved_window_policy = "channel"`, `tp_stat = "sum_mean"`。
- sunscanはTP-only snapshotを読み、`data/tp/...` tableの `tp_mean` を1-channel信号として解析する。

## 2026-05-12 追補: focused test修正

per-window override testの呼び出し関数名を `resolve_spectral_recording_setup` に修正した。

## 2026-05-12 追補: NANTEN2 converter base cleanup

- NANTEN2 converter analysisのactive `planet = "Ori-KL"` をコメントアウト。
- active `spectral_name = "xffts-board1"` をコメントアウト。
- Sun TP sunscan analysisでは `[sunscan_analysis] planet = "sun"` を明示。
