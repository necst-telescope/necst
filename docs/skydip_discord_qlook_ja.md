...
コードで表示し、数値マトリクスは`md`コードブロック内に置く。`tau`、`Tsys0`、`Trx`は
値と誤差を別列（`tau`/`e_tau`、`Tsys0`/`e_Tsys`、`Trx`/`e_Trx`）で表示する。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| xffts-board1 | Band 6 USB | WARN | 0.308 | 0.327 | 29466.300 | n/a | 21578.300 | 123.400 | 8.750 | 5 |
```

同梱v10スクリプトはTsys0の誤差を返さないため、`e_Tsys`は現状`n/a`になる。
