...
def merge_live_telemetry(
    ...
    cmd_lon = (
        _finite_float(cmd.get("command_lon_deg")) if isinstance(cmd, dict) else None
    )
    cmd_lat = (
        _finite_float(cmd.get("command_lat_deg")) if isinstance(cmd, dict) else None
    )
    enc_lon = (
        _finite_float(enc.get("encoder_lon_deg")) if isinstance(enc, dict) else None
    )
    enc_lat = (
        _finite_float(enc.get("encoder_lat_deg")) if isinstance(enc, dict) else None
    )
...
def render(
    ...
    *,
    ...
    record = compact_value(obs.get("record_name"))
    ...
    lines.append(
        f"│ item {index_label:<10} phase={phase:<8} id={obs_id:<10} role={role:<12}│"
    )
    ...
    if geom.get("start") is not None or geom.get("stop") is not None:
        lines.append(
            f"│ plan : kind={gkind:<12} frame={frame:<8} start={compact_value(geom.get('start'))[:20]:<20}│"
        )
        lines.append(f"│        stop={compact_value(geom.get('stop'))[:47]:<47}│")
    elif geom.get("target") is not None:
        lines.append(
            f"│ plan : kind={gkind:<12} frame={frame:<8} target={compact_value(geom.get('target'))[:20]:<20}│"
        )
    elif geom:
        lines.append(
            f"│ plan : kind={gkind:<12} frame={frame:<8} {compact_value(geom)[:31]:<31}│"
        )
    else:
        lines.append("│ plan : -                                                     │")
    if antenna:
        ...
        if cmd_lon is not None or cmd_lat is not None:
            lines.append(
                f"│ cmd  : lon={compact_value(cmd_lon):<13} lat={compact_value(cmd_lat):<13} frame={compact_value(antenna.get('command_frame')):<8}│"
            )
        if enc_lon is not None or enc_lat is not None:
            lines.append(
                f"│ enc  : lon={compact_value(enc_lon):<13} lat={compact_value(enc_lat):<13} frame={compact_value(antenna.get('encoder_frame')):<8}│"
            )
        if antenna.get("tracking_error_deg") is not None:
            lines.append(
                f"│ err  : tracking_error={compact_value(antenna.get('tracking_error_deg')):<37} deg│"
            )
...
def cmd_once(args: argparse.Namespace) -> int:
    ...
    try:
        live.spin_once(0.05)
        snapshot = apply_dynamic_remaining(
...
def build_state(
    ...
    events = read_recent_events(events_path, events_limit)
    return {
        ...
        "plan": plan or {},
        "rendered": render(
            snapshot if isinstance(snapshot, dict) else None, events, compact=True
        ),
...
body { font-family: -apple-system, BlinkMacSystemFont, \"Segoe UI\", sans-serif; margin: 0; padding: 1rem; line-height: 1.35; }
...
.card { border:1px solid var(--border); border-radius:12px; padding:.75rem; box-shadow:0 1px 4px #0001; }
.card h2 { font-size:1rem; margin:.1rem 0 .5rem; }
.kv { display:grid; grid-template-columns: 9.5rem 1fr; gap:.2rem .5rem; }
.k { color:var(--muted); }
.status { font-weight:700; padding:.1rem .45rem; border-radius:999px; border:1px solid var(--border); }
.status.running { color:var(--ok); } .status.error, .status.aborted { color:var(--err); } .status.cleanup { color:var(--warn); }
...
pre { white-space:pre-wrap; overflow:auto; max-height:18rem; margin:.25rem 0 0; }
table { border-collapse:collapse; width:100%; font-size:.9rem; }
th, td { border-bottom:1px solid var(--border); text-align:left; padding:.25rem; vertical-align:top; }
.plotbox { min-height: 260px; display:flex; align-items:center; justify-content:center; }
.plotbox svg { width:100%; max-height:360px; }
...
<section class=\"card\"><h2>Plan View</h2><div class=\"legend\"><span><i class=\"dot\" style=\"background:#2ca02c\"></i>done</span><span><i class=\"dot\" style=\"background:#ff7f0e\"></i>current</span><span><i class=\"dot\" style=\"background:#bdbdbd\"></i>pending</span></div><div id=\"plotview\" class=\"plotbox small\">-</div></section>
...
function val(v) { if (v === null || v === undefined || v === '') return '-'; if (typeof v === 'object') return JSON.stringify(v); return String(v); }
function kv(id, obj, keys) {
  const el = document.getElementById(id); el.innerHTML = '';
  for (const [label, key] of keys) {
    const raw = typeof key === 'function' ? key(obj) : obj?.[key];
    el.insertAdjacentHTML('beforeend', `<div class=\"k\">${esc(label)}</div><div>${esc(val(raw))}</div>`);
  }
}
function stateClass(s) { return ['running','error','aborted','cleanup'].includes(String(s||'').toLowerCase()) ? String(s).toLowerCase() : ''; }
function pct(plan) {
  ...
  for (const item of raw) {
    ...
    if ((g.kind === 'scan_block' || g.kind === 'scan_block_line') && Array.isArray(g.lines)) {
      for (const line of g.lines) {
        ...
        const child = {...item, geometry: {...line, kind: line.kind || 'scan_block_line', frame: line.frame || g.frame, unit: line.unit || g.unit}, _parent_item_uid: item.item_uid};
...
function statusFor(item, snapshot, events) {
  ...
  // Done events are stronger than live/snapshot current state.  During the
  // short inter-item gap after plan_item_finished, the latest snapshot and ROS
  // control topic can still describe the just-finished scan block/line.  Check
  // event/done ranges first so the Web Plan View follows the same rule as
  // progress-plot and terminal compact output.
...
function sx(x, bounds) { return 35 + (x-bounds.xmin) * 330 / Math.max(1e-9, bounds.xmax-bounds.xmin); }
function sy(y, bounds) { return 205 - (y-bounds.ymin) * 170 / Math.max(1e-9, bounds.ymax-bounds.ymin); }
...
function renderMapSvg(snapshot, items, events) {
  ...
  for (const item of items) {
    const g = geomOf(item); const k=g.kind;
    if (k === 'scan_line' || k === 'scan_block_line') { const a=pair(g.start), b=pair(g.stop); if (a&&b) { rows.push({item,a,b,status:statusFor(item,snapshot,events)}); pts.push(a,b); } }
    if (k === 'point' || k === 'grid_point') { const xy=pair(g.target)||pair(g.reference)||pair(g.offset); if (xy) { rows.push({item,a:xy,b:null,status:statusFor(item,snapshot,events)}); pts.push(xy); } }
  }
  if (!pts.length) return '<div>No numeric geometry. This is normal for target-name-only observations.</div>';
  ...
  const els = rows.map(r => r.b ? `<line x1="${sx(r.a[0],b)}" y1="${sy(r.a[1],b)}" x2="${sx(r.b[0],b)}" y2="${sy(r.b[1],b)}" stroke="${colorFor(r.status)}" stroke-width="${r.status==='current'?4:2}" opacity="${r.status==='pending'?0.55:0.95}"/>` : `<circle cx="${sx(r.a[0],b)}" cy="${sy(r.a[1],b)}" r="${r.status==='current'?6:4}" fill="${colorFor(r.status)}" stroke="${r.status==='current'?'#111':'none'}"/>`).join('');
  ...
  return `<svg viewBox="0 0 400 245" role="img"><rect x="1" y="1" width="398" height="243" fill="none" stroke="#9995"/><line x1="35" y1="205" x2="365" y2="205" stroke="#666"/><line x1="35" y1="35" x2="35" y2="205" stroke="#666"/>${els}<text x="35" y="228" font-size="11">x: ${esc(xmin.toFixed(3))} .. ${esc(xmax.toFixed(3))}</text><text x="250" y="228" font-size="11">done ${counts.done} / current ${counts.current} / pending ${counts.pending}</text></svg>`;
}
function renderSkydipSvg(snapshot, items, events) {
  ...
  return `<svg viewBox="0 0 400 245" role="img"><rect x="1" y="1" width="398" height="243" fill="none" stroke="#9995"/><line x1="35" y1="205" x2="365" y2="205" stroke="#666"/><line x1="35" y1="35" x2="35" y2="205" stroke="#666"/><polyline points="${poly}" fill="none" stroke="#9467bd" stroke-width="2"/>${pts}<text x="35" y="228" font-size="11">sequence step</text><text x="250" y="228" font-size="11">elevation ${esc(ymin.toFixed(1))}..${esc(ymax.toFixed(1))} deg</text></svg>`;
}
async function update() {
  try {
    ...
    document.getElementById('message').innerHTML = s.ok ? '' : '<span class=\"err\">Progress snapshot has errors or is missing.</span>';
    kv('observation', {...obs, state: life.state, elapsed_sec: timing.elapsed_sec, remaining_sec: timing.estimated_remaining_sec, remaining_method: timing.remaining_method, remaining_confidence: timing.remaining_confidence}, [['state','state'], ['type','type'], ['record','record_name'], ['obsfile','obs_file'], ['target','target'], ['elapsed [s]','elapsed_sec'], ['remaining≈ [s]','remaining_sec'], ['ETA method','remaining_method'], ['ETA confidence','remaining_confidence']]);
    document.querySelector('#observation div:nth-child(2)').innerHTML = `<span class=\"status ${stateClass(life.state)}\">${esc(life.state ?? '-')}</span>`;
    kv('plan', plan, [['item_uid','item_uid'], ['index0','index0'], ['index0_end','index0_end'], ['total','total'], ['mode','mode'], ['role','role'], ['label','label'], ['obs_id','obs_id']]);
    document.getElementById('bar').style.width = pct(plan).toFixed(1) + '%';
    renderPlanView(snap, s.plan || {}, s.events || []);
    ...
    kv('geometry', geom, [['kind','kind'], ['frame','frame'], ['unit','unit'], ['target','target'], ['target_name','target_name'], ['reference','reference'], ['offset','offset'], ['start','start'], ['stop','stop'], ['line_index0','line_index0'], ['current_line','current_line_index0'], ['line_total','line_total']]);
