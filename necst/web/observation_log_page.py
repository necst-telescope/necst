...
<style>
:root { --bg:#090f20; --panel:#111a31; --panel2:#17233f; --line:#2b3858; --text:#eaf0ff; --muted:#aab7d6; --accent:#79a5ff; --ok:#57d364; --bad:#ff7373; }
...
(() => {
  ...
  async function getJson(url) { const response=await fetch(url,{cache:'no-store'}); const data=await response.json(); if(!response.ok || data.ok===false) throw new Error(data.reason || ('HTTP '+response.status)); return data; }
  ...
  function buildColumns(rows) {
    const preferred=['utc_iso','enc_az_deg','enc_el_deg','comment','mode','event','action_or_obsfile','result','user','target','record_dir','session_id','target_row_id','temp_C','humidity_pct','pressure_hPa','weather_source'];
  ...
  function updateFilters() {
    ...
    const current=state.event; q('event').innerHTML='<option value="all">Event: all</option>'+events.map(value=>`<option value="${esc(value)}">${esc(value)}</option>`).join(''); q('event').value=events.includes(current)?current:'all'; state.event=q('event').value;
  }
  function updateFiles() {
    const current=state.files.find(file=>file.active); q('csv').innerHTML=state.files.map(file=>`<option value="${esc(file.path)}" ${file.active?'selected':''}>${esc(file.name)}${file.active?' · current':''}</option>`).join('');
  ...
  function copySelection() {
    ...
    const lines=['| '+picked.cols.map(markdown).join(' | ')+' |','| '+picked.cols.map(()=> '---').join(' | ')+' |']; picked.rows.forEach(row=>lines.push('| '+picked.cols.map(col=>markdown(row[col])).join(' | ')+' |'));
  ...
  async function load() { try { const [rows,files]=await Promise.all([getJson('/api/observation-log-rows?limit=5000'),getJson('/api/observation-log-files?limit=500')]); state.rows=Array.isArray(rows.rows)?rows.rows:[]; state.files=Array.isArray(files.files)?files.files:[]; state.columns=buildColumns(state.rows); updateFilters(); updateFiles(); render(); if(state.targetRow) syncEditor(state.targetRow); setMessage(`${state.rows.length} row(s)`); } catch(err) { setMessage(String(err),'bad'); } }
  q('order').addEventListener('change',event=>{state.order=event.target.value;render();}); q('event').addEventListener('change',event=>{state.event=event.target.value;render();}); q('refresh').addEventListener('click',load); q('copy').addEventListener('click',copySelection);
  q('comment').addEventListener('input',()=>{}); q('save').addEventListener('click',async()=>{try{await action('obslog_comment',{target_row_id:state.targetRow||'',comment:q('comment').value});await load();setMessage('Comment saved','ok');}catch(err){setMessage(String(err),'bad');}});
  q('csv').addEventListener('change',async event=>{if(!event.target.value)return;try{await action('obslog_open_existing',{path:event.target.value});await load();setMessage('CSV opened','ok');}catch(err){setMessage(String(err),'bad');}});
  q('newCsv').addEventListener('click',async()=>{try{await action('obslog_new');state.targetRow=null;await load();setMessage('New CSV opened','ok');}catch(err){setMessage(String(err),'bad');}});
